...
import { educationTableInfo,familyTableInfo,personalTableInfo,occupationalTableInfo,marriageRelatedTableInfo,expectedPartnersTableInfo,pledgeTableInfo,contactTableInfo } from "@/constraints/script";
...
const page = ({ params }: { params: { biodataId: string } }) => {
  ...
  useEffect(() => {
    const getBiodata = async () => {
      try {
        const res = await axios.post("/api/user/biodata/usersBiodata", { id: params.biodataId});
  ...
  return (
    <section className="  py-[30px] px-[20px] sm:px-[50px]  max-w-full min-h-screen">
      {biodata &&  <h1 className="text-blue  text-[35px] mb-3">Preview Biodata</h1>}
